...
*)let calculate_ind (ol : operand list) (ind : int) (m : mach) : int64 =
...
let store_data (ol : operand list) (ind : int) (m : mach) (data : int64) :
  ...
  in
    ...
    | Ind1 _ | Ind2 _ | Ind3 _ -> 
      ...
      store_data_helper i data m
    | Imm i ->
        failwith "Can't use immediates as locations"
...
let service (m : mach) (elem : sbyte) : unit =
  ...
  | InsFrag -> 
    m.regs.(rind Rip) <- (Int64.add m.regs.(rind Rip) Int64.one)
  | Byte b -> 
    m.regs.(rind Rip) <- (Int64.add m.regs.(rind Rip) Int64.one)
...
let get_size (option: int)(size: int64)(e: elem): int64 = 
	...
	let data_size (d_size: int64)(d: data): int64 = 
		begin match d with
		| Asciz a -> (Int64.add (Int64.add d_size 1L) (Int64.of_int (String.length a)))
...
let translate (opt: int)((tl, s))(e: elem): tpl list * int64 = 
	...
	begin match opt, e.asm with
	| 0, Text t -> 
		let new_size = Int64.add s (Int64.of_int ((List.length t) * 4)) in
		let addr = (find_lbl tl e.lbl 0) in
		if addr = -1L then ((List.append tl [(e.lbl, s)]), new_size) 
...
let rec replace_operands (tl: tpl list)(ol: operand list): operand list = 
	...
	| Ind3 (Lbl l, r)::tail -> 
		List.append [(Ind3(Lit(find_lbl tl l 1), r))] (replace_operands tl tail)
	| [] -> []
...
let replace_each (tl: tpl list)(s: sbyte list)(i:ins): sbyte list = 
	begin match i with
	| (op, ol) -> 
		List.append s (sbytes_of_ins (op, (replace_operands tl ol)))
	end
...
let replace (opt: int)(tl: tpl list)(sbl: sbyte list)(e: elem): sbyte list = 
	
	let replace_all = replace_each tl in
	let replace_dat = replace_data tl in
...
let assemble (p:prog) : exec =
	...
	let ds = List.fold_left replace_data [] p in
